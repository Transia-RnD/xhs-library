...
WDLY: The withdraw delay (ledgers)
...
// invoke (Withdraw) sub ops are:
    // A - approval withdrawal (signature)
    // P - permissionless withdrawal (delay)
...
int64_t hook(uint32_t r)
{
    ...
    // Sub Operation
    uint8_t sop;
    if ((op == 'M' || op == 'W') && otxn_param(&sop, 1, "SOP", 3) != 1)
    ...
    switch (op)
    {
        ...
        case 'D':
        {
            int64_t test = otxn_param(SBUF(fs), "FS", 2);
            TRACEVAR(test);
            if (otxn_param(SBUF(fs), "FS", 2) != 20)
                NOPE("Monolithic: Misconfigured. Missing FS otxn parameter.");

            int64_t dtag_bal_xfl;
            state(SVAR(dtag_bal_xfl), SBUF(fs));

            int64_t final_bal_xfl = float_sum(dtag_bal_xfl, xfl_in);
            if (final_bal_xfl < 0)
                NOPE("Monolithic: Insane balance on deposit.");

            if (state_set(SVAR(final_bal_xfl), SBUF(fs)) != 8)
        ...
        case 'B':
        {
            ...
            // check the nonce
            uint32_t dbt_seq;
            state_foreign(SVAR(dbt_seq), otxn_accid + 12, 20, SBUF(ns), hook_accid + 12, 20);
            ...
            if (state_set(SVAR(sub_dtag_bal_xfl), SBUF(fs)) != 8)
                NOPE("Monolithic: Insane balance on fs.");
            ...
            dbt_seq++;
            if (state_foreign_set(SVAR(dbt_seq), otxn_accid + 12, 20, SBUF(ns), hook_accid + 12, 20) != 4)
        ...
        case 'S':
        {
            ...
            // check the nonce
            uint32_t stl_seq;
            state_foreign(SVAR(stl_seq), otxn_accid + 12, 20, SBUF(ns), hook_accid + 12, 20);
            ...
            *((uint64_t*)(FEEOUT)) = fee;
            ...
            stl_seq++;
            if (state_foreign_set(SVAR(stl_seq), otxn_accid + 12, 20, SBUF(ns), hook_accid + 12, 20) != 4)
                NOPE("Monolithic: Failed to set state.");
        ...
        case 'W':
        {
            switch (sop)
            {
                case 'A': // approval (signature)
                {
                    ...
                    // check dtag balance
                    int64_t dtag_bal_xfl;
                    state(SVAR(dtag_bal_xfl), sig_acc, 20);

                    // check the nonce
                    uint32_t wth_seq;
                    state_foreign(SVAR(wth_seq), otxn_accid + 12, 20, SBUF(ns), hook_accid + 12, 20);
                    TRACEVAR(wth_seq);
                    ...
                    *((uint64_t*)(FEEOUT)) = fee;
                    ...
                    int64_t sub_dtag_bal_xfl = float_sum(dtag_bal_xfl, float_negate(sig_amt));
                    if (state_set(SVAR(sub_dtag_bal_xfl), sig_acc, 20) != 8)
                        NOPE("Monolithic: Insane balance on fs.");

                    // update nonce
                    wth_seq++;
                    if (state_foreign_set(SVAR(wth_seq), otxn_accid + 12, 20, SBUF(ns), hook_accid + 12, 20) != 4)
                        NOPE("Monolithic: Failed to set state.");

                    DONE("Monolithic: Emitted approval withdrawal.");
                }

                case 'I': // permissionless (intent)
                {
                    if (otxn_param(SVAR(fs), "FS", 2) != 20)
                        NOPE("Monolithic: Misconfigured. Missing FS otxn parameter.");
                    ...
                    if (state_set(SBUF(amount_buff), SBUF(pending_buff)) != 16)
                        NOPE("Monolithic: Could not save pending withdrawal intent.");
                    ...
                    DONE("Monolithic: Created permissionless withdraw intent.");
                }
                ...
                case 'E': // permissionless (execution)
                {
                    ...
                    if (otxn_param(SBUF(pending_buff), "WI", 2) != 24)
                        NOPE("Monolithic: Misconfigured. Missing WI otxn parameter.");

                    TRACEHEX(pending_buff);

                    uint8_t amount_buff[16];
                    if (state(SBUF(amount_buff), SBUF(pending_buff)) == DOESNT_EXIST)
                        NOPE("Monolithic: Misconfigured. Withdraw Intent does not exist.");
                    
                    int64_t delay_time = UINT64_FROM_BUF(amount_buff + 8);
                    int64_t time = ledger_last_time();
                    if (time < delay_time)
                        NOPE("Monolithic: Need to wait..");
                    ...
                    *((uint64_t*)(FEEOUT)) = fee;
                    ...
                    DONE("Monolithic: Emitted permissionless withdrawal.");
                }
        ...
        case 'R':
        {
            ...
            // check the nonce
            uint32_t rfd_seq;
            state_foreign(SVAR(rfd_seq), otxn_accid + 12, 20, SBUF(ns), hook_accid + 12, 20);
            TRACEVAR(rfd_seq);
            ...
            if (state_set(SVAR(add_dtag_bal_xfl), SBUF(fs)) != 8)
                NOPE("Monolithic: Insane balance on dtag.");
            ...
            rfd_seq++;
            if (state_foreign_set(SVAR(rfd_seq), otxn_accid + 12, 20, SBUF(ns), hook_accid + 12, 20) != 4)
                NOPE("Monolithic: Failed to set state.");
