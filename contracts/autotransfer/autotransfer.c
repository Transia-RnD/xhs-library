...
int64_t hook(uint32_t reserved)
{
    ...
    TRACESTR("autotransfer.c: Called.");
    ...
    {
        rollback(SBUF("autotransfer.c: Invalid URITokenID"), __LINE__);
    }
    ...
    *b++ = (drops >> 0) & 0xFFU;
    ...
    if (emit_result > 0)
    {
        accept(SBUF("autotransfer.c: Tx emitted success."), __LINE__);
    }
    accept(SBUF("autotransfer.c: Tx emitted failure."), __LINE__);
