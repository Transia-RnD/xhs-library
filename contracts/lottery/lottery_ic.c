...
int64_t hook(uint32_t reserved)
{
    ...
    uint8_t ic_buffer[48];
    slot(SBUF(ic_buffer), 1);
    if (!BUFFER_EQUAL_20(curr_usd, ic_buffer + 8))
    {
        rollback(SBUF("lottery.c: Invalid Amount `Currency`"), 2);
    }
    if (!BUFFER_EQUAL_20(issuer_usd, ic_buffer + 28))
    {
        rollback(SBUF("lottery.c: Invalid Amount `Issuer`"), 2);
    }
